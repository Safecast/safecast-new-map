[![آخرین نسخه پایدار](https://github.com/Safecast/safecast-new-map/actions/workflows/release.yml/badge.svg)](https://github.com/Safecast/safecast-new-map/actions/workflows/release.yml)
...
دموی آنلاین: [https://simplemap.safecast.org/](https://simplemap.safecast.org/) — گرهٔ شما هم همین‌طور دیده می‌شود.

👉 [صفحه واحد دانلود](https://github.com/Safecast/safecast-new-map/releases) (همه سکوها، آخرین بیلدها)
...
1) [صفحهٔ انتشارها](https://github.com/Safecast/safecast-new-map/releases) را باز کنید و بیلد مناسب سیستم خود را بگیرید.
...
2) در Docker Hub جستجو کنید **Safecast/safecast-new-map** و روی **Run** کلیک کنید (یا فرمان زیر را اجرا کنید):
```bash
...
- آینهٔ simplemap.safecast.org را می‌خواهید؟ یک بار `safecast-new-map -import-tgz-url https://simplemap.safecast.org/api/json/weekly.tgz` را اجرا کنید — آرشیو هفتگی را می‌گیرد، پایگاه‌داده را پر می‌کند و خارج می‌شود تا شروع بعدی آماده باشد.
- ترجیح می‌دهید آرشیو را اول بگیرید؟ [https://simplemap.safecast.org/api/json/weekly.tgz](https://simplemap.safecast.org/api/json/weekly.tgz) را دریافت کنید، با `-import-tgz-path /path/to/weekly.tgz` اجرا کنید و از کپی محلی بهره ببرید.
...
- **PostgreSQL (`pgx`)** — سریع‌ترین و مناسب برای چند کاربر. نمونه: `safecast-new-map -db-type pgx -db-conn postgres://USER:PASS@HOST:PORT/DATABASE?sslmode=allow -import-tgz-url https://simplemap.safecast.org/api/json/weekly.tgz`
- **DuckDB / SQLite / Chai** — راهکار فایل‌محور برای یک کاربر. نوشتن همزمان می‌تواند تضاد ایجاد کند؛ برای نقشه‌های شخصی بهتر است. نمونه: `safecast-new-map -db-type duckdb -import-tgz-url https://simplemap.safecast.org/api/json/weekly.tgz`
...
Safecast‑Isotope‑Map از تلاش‌های میدانی **Dmitry Ignatenko** الهام گرفته و از **Rob Oudendijk** و **Safecast** تاثیر پذیرفته است. داده‌های باز از جوامع AtomFast و Radiacode این نقشه را کاربردی نگه می‌دارند. اگر نقشه بتواند حتی یک جان را نجات دهد، بیهوده ساخته نشده است.
