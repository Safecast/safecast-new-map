[![最新稳定版构建](https://github.com/Safecast/safecast-new-map/actions/workflows/release.yml/badge.svg)](https://github.com/Safecast/safecast-new-map/actions/workflows/release.yml)
...
在线演示：[https://simplemap.safecast.org/](https://simplemap.safecast.org/) —— 你的节点看起来也是这样。

👉 [统一下载页](https://github.com/Safecast/safecast-new-map/releases)（所有平台，最新构建）
...
### 方案 1. 二进制（推荐）
1) 打开 [发布页](https://github.com/Safecast/safecast-new-map/releases) 并下载适合你的系统的构建。
2) 赋予可执行权限并运行：
```bash
...
2) 在 Docker Hub 搜索 **Safecast/safecast-new-map** 并点击 **Run**（或执行命令）：
```bash
...
- 想要 simplemap.safecast.org 的镜像？运行一次 `safecast-new-map -import-tgz-url https://simplemap.safecast.org/api/json/weekly.tgz` —— 它会下载每周归档、填充数据库后退出，这样下一次启动就自带数据。
- 想先下载归档？获取 [https://simplemap.safecast.org/api/json/weekly.tgz](https://simplemap.safecast.org/api/json/weekly.tgz)，用 `-import-tgz-path /path/to/weekly.tgz` 启动，使用本地副本。
...
### 🛢️ 导入和日常使用的数据库选择
- **PostgreSQL (`pgx`)** —— 速度最快，适合多人写入。示例：`safecast-new-map -db-type pgx -db-conn postgres://USER:PASS@HOST:PORT/DATABASE?sslmode=allow -import-tgz-url https://simplemap.safecast.org/api/json/weekly.tgz`
- **DuckDB / SQLite / Chai** —— 适合单用户的文件型方案。并发写入会冲突，适合个人地图。示例：`safecast-new-map -db-type duckdb -import-tgz-url https://simplemap.safecast.org/api/json/weekly.tgz`
...
Safecast‑Isotope‑Map 受 **Dmitry Ignatenko** 的野外研究启发，并深受 **Rob Oudendijk** 和 **Safecast** 影响。来自 AtomFast 和 Radiacode 社区的开放数据让地图保持实用。如果地图能拯救哪怕一条生命，它的存在就有意义。
